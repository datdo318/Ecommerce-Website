...
function addCart(productId)
{
    $.ajax({
        ...
        success: function (response){
            ...
            $('.cart-price').text('đ' + numberWithCommas(response['total']));
            $('.select-total h5').text('đ' + numberWithCommas(response['total']));
            $('.pro-qty input:text').val(response['cart'].qty);
            $('.pro-qty input').data('rowid', response['cart'].rowId);
            ...
            if (cartHover_existItem.length) {
                cartHover_existItem.find('.product-selected p').text('đ' + numberWithCommas(response['cart'].price) + ' x ' + response['cart'].qty);
            }else{
                let row = $("<tr data-rowId=" + response['cart'].rowId  + ">" + "</tr>");
                let image = $('<td class="si-pic"> <img style="height:70px" src="front/img/products/' + response['cart'].options.images[0].path + '" alt=""></td>');
                let price = $('   <td class="si-text">' +
                '       <div class="product-selected">' +
                '           <p>đ' + numberWithCommas(response['cart'].price) + ' x ' + response['cart'].qty + '</p>' +
                ...
                '   </td>')
                cartHover_tbody.append(row.append([image, price, icon]));
                // let newItem = 
                ...
                //     '   </td>\n' +
                //     '</tr>'; 
                //     cartHover_tbody.html(newItem);                
            }

            alert('Thêm vào giỏ hàng thành công!\nMặt hàng: ' + response['cart'].name);
...
function removeCart(rowId)
{
    $.ajax({
        ...
        success: function (response){
            ...
            $('.subtotal span').text(numberWithCommas(response['subtotal']));
            $('.cart-total span').text(numberWithCommas(response['total']));
    ...
    $.ajax({
        ...
        success: function (response){
            ...
            if(qty === 0){
                ...
                cart_existItem.find('.total-price').text('đ' + numberWithCommas(response['cart'].price * response['cart'].qty));
            }
